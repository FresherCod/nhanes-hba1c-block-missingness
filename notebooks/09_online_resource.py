...
rows = [
    ...
    ("Post hoc", "Self-reported weight and height", "WHD020, WHD010 (P_WHQ / WHQ_L)", "lb → kg, in → cm; BMI derived"),
]
for r in rows:
    w("| " + " | ".join(r) + " |")
...
for r in [("Age <40 / 40–49 / 50–59 / ≥60 years", "0 / 1 / 2 / 3", "RIDAGEYR"),
          ...
          ("History of hypertension", "1", "BPQ020 = yes or measured BP ≥140/90 mmHg"),
          ("Overweight / obese / extremely obese", "1 / 2 / 3",
           "BMI 25–<30 / 30–<40 / ≥40, or waist (inches) 37–<40 / 40–<50 / ≥50 (men), 31.5–<35 / 35–<49 / ≥49 (women)"),
...
w("\nThe reported M-AUG predictor averages the predicted probabilities of the five seeds.\n")
...
w("The 2021–2023 design has 15 pseudo-strata, each with two pseudo-PSUs. For stratum *h* (h = 1, …, 15), a replicate "
  "weight is formed by multiplying the sample weight by 2 for participants in the first pseudo-PSU of stratum *h* and "
  "by 0 for those in the second, leaving all other strata unchanged. Every quantity θ (Brier skill, AUROC, "
...
w(f"The lock file (LOCK_protocol_v0.4.json) was created on {lock['created']} and records SHA-256 hashes of the "
  "protocol, decision log, source code, stage A/B scripts, trained models and configuration. The validation cycle "
  "was analysed once (stage B, 2026-09-24), and the hash of the resulting prediction file is recorded in the "
  "post-lock log. The lock is internal to the project and is not an independent preregistration. Code, protocol "
